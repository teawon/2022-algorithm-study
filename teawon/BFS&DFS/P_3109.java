 ...
 * 실패했던 방법
 * 
 * 1) DFS를 통해 각 노드의 위치에서 갈 수 있는 경로가 있는지 확인 후 갈수없다면 경로를 막는다.
 * 2) 새롭게 만들어진 지도에서 y축마다 최소 폭을 계산해서 해당 폭의 최솟값이 경로의 개수라고 생각했다.
 *    . x x    그러나 옆과 같이 3열에서 2개의 경로가 카운팅되지만 실제 4행 3열 , 3행 3열의 값은 하나의 경로에 속해서 카운팅이 더된다
 *    . . x
 *    . . .
 *    . x .
 * 
